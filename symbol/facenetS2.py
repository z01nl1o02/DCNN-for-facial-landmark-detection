...
class FACENET_S2(mx.gluon.nn.Block):
    def __init__(self, outputNum, verbose=False, **kwargs):
        super(FACENET_S2,self).__init__(**kwargs)
        ...
        with self.name_scope():
            ...
            layers.append( mx.gluon.nn.MaxPool2D( pool_size=(2,2), strides=2 ) )
            layers.append( mx.gluon.nn.Dense(60, activation ='tanh') )
...
def get_symbol(outputNum,ctx,verbose = False,**kwargs):
    if verbose:
        net = FACENET_S2(outputNum,verbose = True)
        net.initialize(ctx=[mx.cpu()])
    else:
        net = FACENET_S2(outputNum)
